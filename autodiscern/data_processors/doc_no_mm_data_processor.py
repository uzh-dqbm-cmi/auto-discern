...
def doc_no_mm_data_preprocessor(data_dict):
...
if __name__ == "__main__":
    ...
    file_name = dm.cache_data_processor(raw_data_dict, doc_no_mm_data_preprocessor, tag=TAG,
                                        enforce_clean_git=False)
    print("--> Completed caching {}".format(file_name))

    # re-load the processing func and re-run it on an input
    reloaded_data_processor = dm.load_cached_data_processor(file_name)

    # test the data
    retreived_data = reloaded_data_processor.data

    # test the code
    retrieved_code = reloaded_data_processor.view_code()

    # test the processing func
    example_key = list(raw_data_dict.keys())[0]
    example_item = {example_key: raw_data_dict[example_key]}
    result = reloaded_data_processor.rerun(example_item)
